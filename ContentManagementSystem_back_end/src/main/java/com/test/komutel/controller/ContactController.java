package com.test.komutel.controller;
...
import com.test.komutel.repository.ContactRepository;
	...
	public ResponseEntity<Contact> createTutorial(@RequestBody Contact contact) {
		try {
			Contact _contact = contactRepository.save(new Contact(contact.getNom(), contact.getAdress(),
					contact.getVille(),contact.getPays(), contact.getTelephone(), contact.getDebut_contrat()));
